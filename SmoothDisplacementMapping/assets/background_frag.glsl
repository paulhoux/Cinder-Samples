...
void main() {
	// convert rgb to yic 
	vec3 yic = rgb2yiq * texture2D( texture, gl_TexCoord[0].st ).rgb; 

	// calculate new hue
	float h = hue + atan( yic.b, yic.g );

	// convert yic to rgb
	float chroma = sqrt( yic.b * yic.b + yic.g * yic.g );
	vec3 rgb = yiq2rgb * vec3( yic.r, chroma * cos(h), chroma * sin(h) );
